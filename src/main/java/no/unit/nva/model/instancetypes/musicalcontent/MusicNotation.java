...
public class MusicNotation extends NonPeerReviewedPaper {
    ...
    public String getFormattedIsmn() {
        return formatIsmnStringWithHyphens(ismn);
    }
    ...
    private String formatIsmnStringWithHyphens(String ismn) {
        return String.join(ISMN_HYPHEN,
                formatIsmnPrefix(ismn),
                extractIsmnRegistrant(ismn),
                extractIsmnItem(ismn, extractIsmnRegistrant(ismn)),
                extractIsmnCheckbit(ismn)
        );
    }

    private String extractIsmnCheckbit(String ismn) {
        return ismn.substring(CHECKBIT_ISMN_13);
    }

    private String extractIsmnItem(String ismn, String registrant) {
        return ismn.substring(PREFIX_END + registrant.length(), CHECKBIT_ISMN_13);
    }

    private String formatIsmnPrefix(String ismn) {
    ...
    private String extractIsmnRegistrant(String ismn) {
    ...
    private String extractBareIsmn(String candidate) throws InvalidIsmnException {
        List<Integer> bareCandidate = extractCandidateIntegers(candidate);
        new IsmnValidator().validate(candidate);
        return integerListToString(bareCandidate);
    }

    private String integerListToString(List<Integer> bareCandidate) {
        return bareCandidate.stream()
                .map(String::valueOf)
                .collect(Collectors.joining());
    }

    private List<Integer> extractCandidateIntegers(String candidate) {
        return candidate.startsWith(ISMN_10_PREFIX)
                ? convertToIntegerList(candidate.replace(ISMN_10_PREFIX, ISMN_13_PREFIX))
                : convertToIntegerList(candidate);
    }

    private List<Integer> convertToIntegerList(String candidate) {
